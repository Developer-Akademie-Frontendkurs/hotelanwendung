...
Die Branches sind chronologisch nach ihrem ersten Commit sortiert. Inhaltlich bauen sie aufeinander auf: `spa-struktur` setzt auf `project-setup` auf, `testing-spike` auf `spa-struktur`, `startseite-erstellen` auf `testing-spike`, `buchungs-seite` auf `startseite-erstellen` und `datenbank-anbindung` auf `buchungs-seite`.
